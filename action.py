...
def get_warehouse_id(product_id): # good to go
    connection,cursor = create_connection()
    try:
        cursor.execute("SELECT WHID FROM PRODUCTS WHERE PRODID = :PRODUCT_ID",PRODUCT_ID = product_id)
...
def insert_order(order_id,cust_id,connection,cursor):    # good to go
    ...
    """
    try:
        # cursor.execute(find_sql_cust, [cust_id])
        # row = cursor.fetchone()
        # a situation might arise: that we found a customer, and as soon as we found a customer, that customer is deleted.
        cursor.execute(insert_sql,data)
...
def deleting_cust(cust_id):    # good to go
    ...
    """
    try:
        ispresent = findcustomer(cust_id)
        if ispresent[0]:
                cursor.execute(del_sql,[cust_id])
            ...
            traceInfo(f"customer id:{cust_id} already not active")
    except oracledb.DatabaseError as e:
        error, = e.args
        # if "ORA-00060: deadlock" in error.message:
        #     i = 0
        #     while True:
        #         i+=1
        #         # time.sleep(0.1)
...
def deleting_order(order_id):   # good to go
    connection, cursor = create_connection()
